...
There are many software development kits for microcontroller applications, such as STM32 CubeMX, TI Code Composer Studio, Keil MDK, etc.
They provide GUI interfaces to initialize, build and debug the code.
In addition to this, they applies an abstraction to the register level by using high-level libraries and OS integration.
All this makes the learning curve steep, enables fast initial development and allows a better portability in the same vendor devices.

On the other hand, hiding the low level implementation details the high abstraction levels reduce the level of control on the MCU.
Sometimes changing the initial settings is not straight-forward and you may have to make the change in the GUI and re-generate the design.
They may also prevent to have an optimized code.
Even more, they may need a paid license.

An alternative method -in fact it should be the default way- is the bare-metal programming.
In this way, any abstraction is not used and direct access to the MCU registers is done.
Brings better control on the implementation and comprehension.
Moreove, text based development is more reliable, repeatable comparing to GUI clicking based development.
And make feels more comfortable hardware engineers :)

In this post, I will share my first experiments with bare-metal programming.
I will show work environment, based on official documents and codes.
I will point the core documents. 
Minimum set of files (sources, headers, Makefile), and tools.
All is open source tools which makes the development platform free.
...
[ARM CMSIS_5](https://arm-software.github.io/CMSIS_5/Core/html/index.html)
...
[STM32CubeF0](https://github.com/STMicroelectronics/STM32CubeF0)
